...
contract CrossChainTokenHolder is TokenSender, TokenReceiver {
    ...
    function sendCrossChainDeposit(
        uint16 targetChain,
        address targetCrossChainTokenHolder,
        address recipient,
        uint256 amount,
        address token
    ) public payable {
        ...
        sendTokenWithPayloadToEvm(
            targetChain, 
            targetCrossChainTokenHolder, // address (on targetChain) to send token and payload to
            payload, 
            0, // receiver value
            GAS_LIMIT, 
